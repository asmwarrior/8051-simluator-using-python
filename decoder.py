...
codes = ['NOP', 'RR', 'INC', 'RRC', 'DEC', 'RL', 'ADD', 'RLC', 'ADDC', 'ORL', 'ANL', 'XRL', 'MOV', 'MOVC', 'DIV', 'SUBB', 'MUL', 'CPL', 'PUSH', 'CLR', 'SWAP', 'XCH', 'POP', 'SETB', 'DA', 'XCHD', 'MOVX']
...
def splitinst(instr,opcode,lenopcode):
...
def decode(instruction,opcode,lenopcode,count):
	instsplit = splitinst(instruction.upper(),opcode,lenopcode)
	#--- Decode the splitted instruction ['MOV','A','7A'] to ['E5','7A'] ---#
	temp2 = []
	opctemp = []
	if len(instsplit) == 1:
		try:
			opctemp += [opc_dict[instsplit[0]]]	#--- Give exception on key error ---#
		except KeyError:
			UC.flag = 1
...
#		print opctemp
		return opctemp
	elif instsplit[0] in codes:	#--- Check weather valid nemonic or not ---#
		temp = instsplit[0]
		for n in range(1,len(instsplit)):
			if instsplit[n] in regis:
				temp = temp + instsplit[n]
			elif instsplit[n][0] == '#':	#--- for data ---#
				temp = temp + '#'
				if len(instsplit[n]) == 3 and instsplit[1] != 'DPTR':
					temp2.append(instsplit[n][1:3])
				elif len(instsplit[n]) == 4 and instsplit[n][1] == '0' and instsplit[1] != 'DPTR':	#--- decode data of kind #0F3 ---#
					temp2.append(instsplit[n][2:4])
				elif len(instsplit[n]) == 5 and instsplit[1] == 'DPTR':	#--- For DPTR ---#
					temp2.append(instsplit[n][1:3])
					temp2.append(instsplit[n][3:5])
				elif len(instsplit[n]) == 6 and instsplit[n][1] == '0' and instsplit[1] == 'DPTR':	#--- decode data of kind #0F334 for DPTR ---#
					temp2.append(instsplit[n][2:4])
				...
				elif len(instsplit[n]) == 3 and instsplit[n][0] == '0':	#--- decode code addr of kind 0F3 ---#
					temp2.append(instsplit[n][1:3])
				else:
					UC.flag = 1					
					UC.logcnt = UC.logcnt+1
					UC.log[UC.logcnt] = "Error typing address " + instsplit[n] + " in line %d" %(count)	#---- Error in address ---#
#					print "Error typing address " + instsplit[n]

		opctemp += [opc_dict[temp]] + temp2
#		print opctemp
		return opctemp
	elif instsplit[0] in jumps:	#--- Check weather valid jump nemonic or not ---#
		temp = instsplit[0]
		for n in range(1,len(instsplit)):
			if instsplit[n] in regis:
				temp = temp + instsplit[n]
				...
				if len(instsplit[n]) == 3:
					temp2.append(instsplit[n][1:3])
				elif len(instsplit[n]) == 4 and instsplit[n][1] == '0':	#--- decode data of kind #0F3 ---#
					temp2.append(instsplit[n][2:4])
				else:
					UC.flag = 1					
					UC.logcnt = UC.logcnt+1
				...
				if instsplit[n] in sfrs_list:	#--- Decode SFR ---#
					...
					UC.logcnt = UC.logcnt+1
					UC.log[UC.logcnt] = instsplit[n] + " is label in line %d?" %(count)	#--- check for label ---#
		...
		UC.flag = 1
		UC.logcnt = UC.logcnt+1
		UC.log[UC.logcnt] = "Invalid Instruction" + " in line %d" %(count)	#---- Error in instruction ---#
...
def decoderinst(asm_code_list):
	opcode = []
	count = 0
	for ainstr in asm_code_list:
		if (asm_code_list[count]==''):
			pass
		else:
			opcode += decode(ainstr,opcode,len(opcode),count+1)
		if UC.flag == 1:
			break		
		count += 1
	print(label_dict)
